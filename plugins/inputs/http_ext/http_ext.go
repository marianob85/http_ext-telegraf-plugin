...
func (h *HttpExt) gatherServer(
	...
	if h.Name == "" {
		msrmnt_name = "http_ext"
	} else {
		msrmnt_name = "http_ext_" + h.Name
	}
...
// Sends an HTTP request to the server using the HttpExt object's HTTPClient.
...
//     error : Any error that may have occurred
func (h *HttpExt) sendRequest(serverURL string) (string, float64, error) {
	// Prepare URL
...
func (j *MapInterfaceParser) initDebug(debug bool, server string) {
	if debug {
		...
		j.file, _ = os.Create("http_ext_debug_" + u.Host + "-" + u.RawQuery + ".txt")
	}
}
...
func init() {
	inputs.Add("http_ext", func() telegraf.Input {
		return &HttpExt{
